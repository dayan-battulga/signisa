...
# NOT aliased on purpose: garbage (ASL-LEX 'trash' is the BASKET sign; 'throw_away'
# plausible but unconfirmed), store (would resolve to shop_1 = SHOPPING, wrong variant),
# sleepy (TIRED is a different sign), jeans, say.
ALIAS = {
    "another": "other", "beside": "nextto", "callonphone": "telephone", "dad": "father",
    "eye": "eyes", "food": "eat", "glasswindow": "window",
    ...
    # Same sign as another Kaggle gloss (label collisions — visually identical pairs):
    "wake": "awake", "kitty": "cat", "puppy": "dog", "nap": "sleep",
}
...
def load_asllex(path: Path) -> dict[str, pd.Series]:
    ...
    for _, row in lex.sort_values("EntryID", key=lambda c: c.str.lower()).iterrows():
...
def main() -> None:
    ...
    # Alias-aware: the overlap is about sign identity, so grandpa counts via 'grandfather'.
    in_wlasl = {s: normalize(s) in wlasl_glosses or ALIAS.get(s) in wlasl_glosses for s in signs}
...
def write_report(path, signs, counts, matched, unmatched, in_wlasl, strong, any_adj,
                 clusters, curriculum_df, n_participants) -> None:
    ...
    lines = [
        ...
        "## Dataset coverage\n",
        f"- {counts.n_examples.sum()} sequences, 250 signs, {n_participants} participants.",
        ...
        "  - wake/kitty/puppy/nap are aliased to awake/cat/dog/sleep — visually identical sign pairs "
        "within the 250, so they appear as maximal strong confusables of each other.\n",
        "## WLASL-2000 overlap\n",
    ...
    lines.append("|---|---|---|---|---|---|---|")
    for s in signs:
        entry = str(matched[s]["EntryID"]).strip() if s in matched else "-"  # some EntryIDs have stray spaces
